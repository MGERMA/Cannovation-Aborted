...
class Editor(DirectObject):
	def __init__(self):
		...
		self.currentTexLayer=1 #the layer we are currently painting.. if we are painting a texture
		self.textureForLayer=[0,0]  #textures for texture layers
		...
		#strange files in the map directory will make problems
		self.maps=[]
		dirList=os.listdir(Filename(path+"maps/").toOsSpecific())
		for fname in dirList:
			self.maps.append(fname)
			
			
		#where the map is saved?
		self.saveName="maps/map"
		
		# check for specific instructions regarding filename
		for key in range(len(sys.argv)):
			if sys.argv[key] == "-c":
				if (os.path.exists("maps/"+sys.argv[key+1])):
					print "WARNING: ",sys.argv[key+1], "already exists. Continuing with new file"
					...
					self.saveName = "maps/"+sys.argv[key+1]
					print "INFO: ",sys.argv[key+1], "map successfully created."
		...
		wp=WindowProperties()
		wp.setTitle("SLEEP3D by Wezu v1.01   ("+str(self.saveName)+")")        
		wp.setSize(800,600)
		...
		dlight.setColor(VBase4(.9, .9, .9, 1))        
		...
		self.TexPlane[0].setTexture(self.TexPlane[0].findTextureStage('Tex1'), temp_textures, 1)    
		self.TexPlane[0].setTexture(self.TexPlane[0].findTextureStage('Tex2'), loader.loadTexture('walk_map.png'), 1)        
		self.TexPlane[0].setZ(-0.5)    #the Z is a hack, to avoid confilts with the texBrush, depth offset didn't work here    
		self.TexPlane[0].hide()
		
		self.TexPlane.append(self.texturePlane.copyTo(self.altTexRender))  
		temp_textures=Texture()
		temp_textures.read('blank_mask.png')
		temp_textures.setFormat(Texture.FAlpha)  
		self.TexPlane[1].setTexture(self.TexPlane[1].findTextureStage('Tex1'), temp_textures, 1)        
		self.TexPlane[1].setTexture(self.TexPlane[1].findTextureStage('Tex2'), loader.loadTexture(path+'textures/'+self.textures[0]), 1)        
		self.TexPlane[1].setZ(-1)
		self.TexPlane[1].setDepthOffset(1)       
		
		#walk-map
		#self.walkMapPlane =self.texturePlane.copyTo(self.altTexRender)         
		#self.walkMapPlane.setTexture(self.walkMapPlane.findTextureStage('Tex1'), temp_textures, 1)        
		#self.walkMapPlane.setTexture(self.walkMapPlane.findTextureStage('Tex2'), loader.loadTexture('full_mask.png'), 1)
		#self.walkMapPlane.setColor(0,0,0,1)        
		#self.walkMapPlane.setZ(-0.5)
		#self.walkMapPlane.hide()
		...
		#a list of dirs that can be loaded, a scrolled frame is in need        
		self.loadScrollFrame = DirectScrolledFrame(canvasSize = (138,0,0,-(len(self.maps)-1)*26),
							  frameSize = (138,0,0,-512),                              
		...
		self.loadScrollFrame.verticalScroll['value']=1
		self.loadScrollFrame.verticalScroll['incButton_relief']=None
		self.loadScrollFrame.verticalScroll['incButton_state'] = DGG.DISABLED
		self.loadScrollFrame.verticalScroll['decButton_relief']=None
		self.loadScrollFrame.verticalScroll['decButton_state'] = DGG.DISABLED        
		self.loadScrollFrame.setPos(0,0,-32)
		self.loadScrollFrame.hide()
		self.loadFrames=[]
		for i in xrange(len(self.maps)):   
			self.loadFrames.append(DirectFrame(frameSize=(-128, 0, 0, 24),
									frameColor=(0, 0, 0, .4),
									text=self.maps[i],
									text_scale=15,
									text_fg=(1, 1, 1, 1),
									...
									text_pos=(-128+5,9,0),
									state=DGG.NORMAL,
									parent=self.loadScrollFrame.getCanvas()))
			self.loadFrames[i].setTransparency(TransparencyAttrib.MAlpha)
			self.loadFrames[i].setPos(138,0,-26*(i+1))  
			self.loadFrames[i].bind(DGG.B1PRESS,  self.loadMap,[self.maps[i]])    
		#load
		self.loadFrame=DirectFrame(frameSize=(-64, 0, 0, 64),
									frameColor=(1, 1, 1, 0.8),
									frameTexture='save.png',                                   
									state=DGG.NORMAL,
									parent=pixel2d)                               
		self.loadFrame.setTransparency(TransparencyAttrib.MAlpha)
		self.loadFrame.setPos(self.winX,0,-128)
		self.loadFrame.bind(DGG.B1PRESS, self.showLoadFrame) 
		#self.loadFrame.hide()
		...
		self.textureFrames.append(DirectFrame(frameSize=(-64, 0, 0, 64),
									...
									frameTexture='footsteps.png',
									state=DGG.NORMAL,
									parent=pixel2d))                                                 
		self.textureFrames[0].setPos(self.winX,0,-128) 
		self.textureFrames[0].bind(DGG.B1PRESS, self.selctWalkMap)         
		...
		self.texPointFrame.setPos(self.winX-64,0,-192)  
		...
		for i in xrange(len(self.models)):   
			...
			self.objectFrames[i].setPos(self.maxWidth+12,0,-26*(i+1))  
		...
		for key in range(len(sys.argv)):
			if sys.argv[key] == "-m":
				if (os.path.exists("maps/"+sys.argv[key+1])):
					self.loadMap(sys.argv[key+1])
					print "INFO: ",sys.argv[key+1], "map successfully loaded."
				else:
					print "WARNING: ",sys.argv[key+1], "didn't exists. Continuing with new file"
			if sys.argv[key] == "-h":
				print "usage:\n"
				print "main.py -m <mapname>  -  modify an already existent map"
				print "main.py -c <mapname>  -  creates a new map"
				print "main.py -h            -  shows this help"
				exit(0)
	...
	def showLoadFrame(self,notUsed=None):
		print self.maps
		if self.loadScrollFrame.isHidden():
			self.loadScrollFrame.show()
		else:    
			self.loadScrollFrame.hide()        
	
	def loadMap(self, file, notUsed=None):
		self.loadFrame.hide()
		self.loadScrollFrame.hide() 
		if os.listdir(Filename(path+self.saveName).toOsSpecific()) == []:
			os.rmdir(Filename(path+self.saveName).toOsSpecific())
			
		self.terrain.setHeightfield(path+'maps/'+file+'/hf.png')  
		self.terrain.update() 
		self.saveName='maps/'+file    
		if os.path.exists(path+self.saveName+"/hf.png"):    
			self.altPlane.setTexture(loader.loadTexture(path+self.saveName+"/hf.png"), 1)        
		if os.path.exists(path+self.saveName+"/0tex.png"):   
			self.TexPlane[0].setTexture(self.TexPlane[0].findTextureStage('Tex1'), loader.loadTexture(path+self.saveName+"/0tex.png"), 1)                                               
		
		wp=WindowProperties()
		wp.setTitle("SLEEP3D by Wezu v1.01   ("+str(self.saveName)+")")                
		base.win.requestProperties(wp)
		
		data=open(Filename(path+'maps/'+file+'/data.json').toOsSpecific()).read()
		imported=json.loads(data)              
		for item in imported:
			if item.has_key(u'textures'): #textures    
				print item['textures']
				for i in xrange(len(item['textures'])):
					print i
					if i==0: #first layer, and button, mst be ther, no mask for this layer                        
						self.textureFrames[1]['frameTexture']=path+'textures/'+item['textures'][0] 
						self.TexPlane[1].setTexture(self.TexPlane[1].findTextureStage('Tex2'), loader.loadTexture(path+'textures/'+item['textures'][i]), 1)                                           
						self.textureForLayer[1]=self.textures.index(item['textures'][i])                    
					else: #no button, no texture plane
						if not i==1:
							self.textureFrames.append(DirectFrame(frameSize=(-64, 0, 0, 64),
																frameColor=(1, 1, 1, 1),
																frameTexture='plus.png',
																state=DGG.NORMAL,
																parent=pixel2d))                                                 
						self.textureFrames[i+1].setTransparency(TransparencyAttrib.MAlpha)                                    
						self.textureFrames[i+1].setPos(self.winX,0,-128-(64*(i+1)))
						self.textureFrames[i+1].bind(DGG.B1PRESS, self.selctLayer,[i+1]) 
						self.textureFrames[i+1]['frameTexture']=path+'textures/'+item['textures'][i]                        
						self.textureFrames[i+1].hide()
						self.TexPlane.append(self.texturePlane.copyTo(self.altTexRender))      
						self.TexPlane[-1].setZ(-1)
						self.TexPlane[-1].setDepthOffset(len(self.TexPlane)-1)  
						temp_textures=Texture()
						temp_textures.read(path+'maps/'+file+'/'+str(i+1)+'tex.png')
						temp_textures.setFormat(Texture.FAlpha)                          
						self.TexPlane[i+1].setTexture(self.TexPlane[i+1].findTextureStage('Tex1'), temp_textures, 1)  
						self.TexPlane[i+1].setTexture(self.TexPlane[i+1].findTextureStage('Tex2'), loader.loadTexture(path+'textures/'+item['textures'][i]), 1) 
						self.textureForLayer.append(0)
						self.textureForLayer[i+1]=self.textures.index(item['textures'][i])
				if len(item['textures']) >1: #plus button?
					self.textureFrames.append(DirectFrame(frameSize=(-64, 0, 0, 64),
														frameColor=(1, 1, 1, 1),
														frameTexture='plus.png',
														state=DGG.NORMAL,
														parent=pixel2d))                                                 
					self.textureFrames[-1].setTransparency(TransparencyAttrib.MAlpha)                                    
					self.textureFrames[-1].setPos(self.winX,0,-128-(64*(len(self.textureFrames)-1)))
					self.textureFrames[-1].bind(DGG.B1PRESS, self.selctLayer,[len(self.textureFrames)-1])                         
					self.textureFrames[-1].hide()                    
				del imported[imported.index(item)]    
			else:
				root=None
				if item['root']=='render':
					root=render
				else:    
					root=render.find('**/'+item['root'])
					if root.is_empty():
						root=render.attachNewNode(item['root'])
				model=loader.loadModel(path+item['model'])
				model.reparentTo(root) 
				#model.setPosHprScale(float(item['X']),float(item['Y']),float(item['Z']),float(item['H']),float(item['P']),float( item['R']),float( item['Sx']),float(item['Sy']),float(item['Sz']))
				model.setPosHprScale(item['X'], item['Y'], item['Z'], item['H'], item['P'], item['R'], item['Sx'], item['Sy'], item['Sz'])
				if not root == render:
					root.flattenStrong() 
			self.export=imported
	...
	def onExitCleaup(self):
		...
		if self.nothingToSave:    
			...
			tex=[]
			for i in xrange(len(self.textureForLayer)):  
				if i>0:
					tex.append(self.textures[self.textureForLayer[i]])                    
				
			if not len(self.export)==0 or len(tex)>0:             
				self.export.append({'textures': tex})
	...
	def togleFrame(self, frame, notUsed=None):
		if frame.getPythonTag('isOpen'):
			...
			for child in frame.getChildren():
				child.show()
	def selctWalkMap(self,notUsed=None):
		self.textureFrames[0]['frameColor']=(1, 1, 1, 1)
		self.TexPlane[0].show()
		self.setBrush(15, None)
		self.brush.setColor(1, 1, 1, 1)
		self.brush.setH(0)
		self.alphaFrame['text']= '1.00'
		self.colorFrame['text']= '1.00'
		self.rotateFrame['text']= '0.00'  
	...
	def setEditMode (self, mode, notUsed=None):
		...
		if mode==1:
			...
			for item in self.textureFrames:                
				item.hide()    
			for i in xrange(16):
		...
		if self.editMode==3:        
			if not self.currentObject==None:       
				...
				self.export.append({'model':self.currentModelName,
									'H':float(self.currentObject.getH()),
									'P':float(self.currentObject.getP()),
									'R':float(self.currentObject.getR()),
									'X':float(self.currentObject.getX()),
									'Y':float(self.currentObject.getY()),
									'Z':float(self.currentObject.getZ()),
									'Sx':float(self.currentObject.getSx()),
									'Sy':float(self.currentObject.getSy()),
									'Sz':float(self.currentObject.getSz()),
	...
	def mouseWheel(self, up):
		if self.mode==1:
			...
			if self.editMode==3:
				...
				if self.currentTexLayer==0:
					if up and color[0]<1:
						self.brush.setColor(1, 1, 1, color[3])
						self.colorFrame['text']= "%.02f" % float(color[0]+0.01)
						self.colorFrame['text']=  "1.00"
					elif color[0]>0:
						self.brush.setColor(0, 0, 0, color[3])            
						self.colorFrame['text']= "0.00"
				else:
					if up and color[0]<1:
