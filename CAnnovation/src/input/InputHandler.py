...
class InputHandler(DirectObject):
	...
	def setActive(self):
		...
		# Main Modifier
		self.accept("w", self.pressKey, ["e"])
		self.accept("w-up", self.releaseKey, ["e"])
		self.accept("q", self.pressKey, ["a"])
		self.accept("q-up", self.releaseKey, ["a"])
		...
		self.accept("a", self.pressKey, ["q"])
		self.accept("a-up", self.releaseKey, ["q"])
		#self.ignore()
